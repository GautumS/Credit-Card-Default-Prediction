...
build_NN <- function(train, test, y_var) {
  x_vars <- setdiff(colnames(train), y_var)
  nn_formula <- as.formula(paste(y_var, "~", paste(x_vars, collapse = " + ")))
  ...
  # Training Accuracy
  train_preds <- compute(nn_model, train[, x_vars])$net.result
  train_classes <- ifelse(train_preds > 0.5, 1, 0)
  print("Training Confusion Matrix:")
  print(table(Predicted = train_classes, Actual = train[[y_var]]))
  
  train_acc <- sum(train_classes == train[[y_var]]) / nrow(train)
  print(paste("Training Accuracy:", train_acc))
  
  # Predict on TEST data 
  test_preds <- compute(nn_model, test[, x_vars])$net.result
  test_classes <- ifelse(test_preds > 0.5, 1, 0)
  print("Test Confusion Matrix:")
  print(table(Predicted = test_classes, Actual = test[[y_var]]))
  
  # Graph the Probability Scores
  hist(test_preds, 
       breaks = 20, 
  ...
  # Calculate Accuracy
  test_acc <- sum(test_classes == test[[y_var]]) / nrow(test)
  return(test_acc)
}
```
...
Neural Netwrok
```{r}
NN_acc <- build_NN(split_int$train, split_int$test, "default")
NN_acc
```
